...
template <typename T> struct rsq_segtree : public segtree<T, T> {
    static constexpr auto range_increment = plus<T>();
    static constexpr auto range_increment_lazy = [](T st, T delta, int s, int e) { return st + delta * (e - s + 1); };
    static constexpr auto range_set = [](T x, T y) { return y; };
    static constexpr auto range_set_lazy = [](T st, T delta, int s, int e) { return delta * (e - s + 1); };
