...
using namespace std;
using ll = long long;
template <typename T = ll, bool Index> pair<vector<T>, vector<int>> longest_path(graph<1, edge, Index>& g,
...
const vector<T>& node_weight = vector<T>()) {
    ...
    function<T(int)> longest_path = [&](int u) {
        if(opt[u] == -1) {
            opt[u] = weight(u);
            for(int e : g[u]) {
                if(opt[u] < weight(u) + 1 + longest_path(g(e).v)) {
                    opt[u] = weight(u) + 1 + longest_path(g(e).v);
                    arg[u] = e;
                }
            }
        }
